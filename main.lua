...
if pg:FindFirstChild("H3755X") then pg.H3755X:Destroy() end
...
-- ============ DISCORD AU-DESSUS DE LA TÊTE ============
local function ShowDiscordOnHead()
    local char = player.Character
    if not char then return end
    local head = char:FindFirstChild("Head")
    if not head then return end
    if head:FindFirstChild("H375Discord") then return end
    local bb = Instance.new("BillboardGui")
    bb.Name = "H375Discord"
    ...
    lbl.Size = UDim2.new(1, 0, 1, 0)
    lbl.BackgroundTransparency = 1
    lbl.Text = "📱 3755X'JR — discord.gg/CGf9Cs3Kr"
    lbl.TextColor3 = RED
    lbl.TextSize = 16
    lbl.Font = Enum.Font.GothamBold
    lbl.TextStrokeTransparency = 0
...
topBar.Size = UDim2.new(0, 380, 0, 40)
topBar.Position = UDim2.new(0.5, -190, 0, 20)
...
topLayout.Padding = UDim.new(0, 15)

local stealLabel = Instance.new("TextLabel", topBar)
stealLabel.Size = UDim2.new(0, 100, 0, 24)
stealLabel.BackgroundTransparency = 1
stealLabel.Text = "STEAL 0%"
stealLabel.TextColor3 = RED
...
fpsLabel.TextColor3 = TEXT_WHITE
fpsLabel.TextSize = 14
fpsLabel.Font = Enum.Font.GothamBold

local pingLabel = Instance.new("TextLabel", topBar)
pingLabel.Size = UDim2.new(0, 110, 0, 24)
...
local Tabs = {
    ...
    COMBAT = {},
    STEAL = {
        {"MT", "AUTO STEAL", false, function(v) end},
        {"MB", "RADIUS 60", function() end}
    },
    MOVEMENT = {
    ...
    INTERFACE = {
        {"MB", "FPS BOOST", function() end}
    },
    KEYBINDS = {
        {"MB", "RESET KEYS", function() end}
    }
}

local TabNames = {"SPEED", "COMBAT", "STEAL", "MOVEMENT", "MISC", "VISUAL", "INTERFACE", "KEYBINDS"}
local tabButtons = {}
...
local function LoadTab(tabName)
    for _, c in ipairs(content:GetChildren()) do
        if c:IsA("Frame") or c:IsA("TextButton") then
            if c.ClassName ~= "UIListLayout" then c:Destroy() end
        end
    end
    ...
    if tabButtons[tabName] then
        tabButtons[tabName].BackgroundColor3 = DARK_RED
        tabButtons[tabName].TextColor3 = TEXT_WHITE
    end
    panelTitle.Text = tabName
    for _, d in ipairs(Tabs[tabName] or {}) do
        if d[1] == "MB" then
            MB(d[2], d[3])
        elseif d[1] == "MT" then
            MT(d[2], d[3], d[4])
